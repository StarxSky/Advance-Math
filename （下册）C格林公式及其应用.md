...
> 注意⚠️：对于复连通区域，格林公式的右端闭曲线积分应该包含沿区域$ D$的全部边界曲线积分，且边界的方向对于区域$ D$来说都是正向


关于格林公式的一个简单的应用： 

使用格林公式求解一个闭区域$ D$的面积： 
在公式中通过取$$ P(x, y) = -y \\ \space \\ Q(x, y) = x $$得到： $$ 2\iint_{D} dx dy = \oint_{L} -y dx + x dy = \oint_{L} xdy - ydx$$最终得到： $$ A = \iint_{D} dx dy = \frac{1}{2} \oint_{L} xdy - ydx$$其中的$ L$为闭区域$ D$的边界曲线.
# 平面上曲线积分与路径无关的条件
