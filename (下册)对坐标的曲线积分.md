...
因此，平面上的曲线弧的两类积分之间具有的关系可以总结如下 $$ \int_{L} P dx + Q dy = \int_{L} (Pcos\alpha + Qcos\beta) ds$$
其中$ \alpha(x, y), \beta(x, y)$ 为曲线弧$ L$ 在点$ M(x, y)$处的切向量的方向角该方法同样可以推广到空间中的有向曲线弧$ \Lambda$上: $$ \int_{\Lambda} Pdx + Qdy + Rdz = \int_{\Lambda} \{{P cos\alpha + Q cos\beta + R cos\gamma} \}ds$$

* 向量方式： $$ \int_{L} A \cdot dr = \int_{L} A \cdot \tau ds $$其中的$ A = (P, Q) $, $\tau = (cos\alpha , cos\beta)$为有向弧 $ L$在点$ (x, y)$处的单位切向量。$dr= \tau ds = (dx, dy)$称为**有向曲线元** 。 也可以记作: $$ \int_{L } A \cdot dr = \int_{L} A_{\tau} ds $$其中，$ A_{\tau}$称为向量$ A$在向量$ \tau$上的投影